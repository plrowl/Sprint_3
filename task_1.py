...
class OnlineSalesRegisterCollector:
    ...
    def check_amount(self):
        total = []
        for item_name in self.name_items:
            total.append(self.__item_price.get(item_name, 0))
    ...
    def twenty_percent_tax_calculation(self):
        twenty_percent_tax = []
        for item_name in self.name_items:
            if self.__tax_rate.get(item_name) == 20:
                twenty_percent_tax.append(item_name)
        total = [] 
        for item in twenty_percent_tax:
            total.append(self.__item_price.get(item, 0))
        if len(self.name_items) > 10:
    ...
    def ten_percent_tax_calculation(self):
        ten_percent_tax = []
        for item_name in self.name_items:
            if self.__tax_rate.get(item_name) == 10:
